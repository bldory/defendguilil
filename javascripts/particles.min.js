$(function () {
  ...
  var buildCollisionMatrix = function () {
    ...
    for (var i = 0; i < cWidth / boxSize; i++) {
      ...
      for (var j = 0; j < cHeight / boxSize; j++) {
        ...
        if (
          j > (cHeight / 2 + fontSize * 1.5) / boxSize ||
          j < (cHeight / 2 - fontSize * 1.5) / boxSize
        ) {
          hitTest[i][j] = 0;
          continue;
        ...
        for (var x = i * boxSize - boxSize; x < i * boxSize + boxSize; x++) {
          for (var y = j * boxSize - boxSize; y < j * boxSize + boxSize; y++) {
            ...
            pixel +=
              data[x * 4 + y * 4 * cWidth] +
              data[x * 4 + y * 4 * cWidth + 1] +
              data[x * 4 + y * 4 * cWidth + 2] +
              data[x * 4 + y * 4 * cWidth + 3];
          }
        }
        hitTest[i][j] = pixel;
      }
  ...
  function batchAdd() {
    for (var i = 0; i < particleCount; i++) {
      var sprite = PIXI.Sprite.fromImage(
        "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAoAAAAKCAYAAACNMs+9AAAAUklEQVR42mP4//8/PxDH//v3dz8yBomB5Bhg4P//f+uB+D8OvB5Z4X/8+H89UQqBzrhPpIn//hOr8D2pbvx7HrcikBw0iKDhmI/FpHyUcCQWAABhzSxEQoKU+gAAAABJRU5ErkJggg=="
      );
      initSprite(sprite);
  ...
  var animate = function (t) {
    ...
    for (var i in sprites) {
      ...
      sprite.position.x +=
        Math.sin(sprite.direction) * (sprite.vx * sprite.scale.x);
      sprite.position.y +=
        Math.cos(sprite.direction) * (sprite.vy * sprite.scale.y);
